...
export class RealtimeIntegration {
  ...
  async initialize(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      return {
        success: false,
        error: 'Failed to initialize real-time integration',
      };
    }
  }
  ...
  addConnection(
    ...
    try {
      ...
      console.log(
        `Real-time connection added: ${connectionId} (user: ${userId || 'anonymous'})`
      );
  ...
  subscribeToEvents(
    ...
    try {
      ...
      connection.subscriptions = Array.from(
        new Set([...connection.subscriptions, ...eventTypes])
      );
      console.log(
        `Connection ${connectionId} subscribed to: ${eventTypes.join(', ')}`
      );
  ...
  unsubscribeFromEvents(
    ...
    try {
      ...
      console.log(
        `Connection ${connectionId} unsubscribed from: ${eventTypes.join(', ')}`
      );
  ...
  sendToUser(
    userId: string,
    event: RealtimeEvent
  ): { success: boolean; sentTo: number; error?: string } {
    try {
      let sentTo = 0;
      ...
      for (const [connectionId, connection] of Array.from(
        ...
        if (
          ...
          this.shouldSendToConnection(connection, event)
        ) {
          this.sendToConnectionInternal(connection, event);
          sentTo++;
        }
      ...
      console.log(
        `Event ${event.type} sent to user ${userId} (${sentTo} connections)`
      );
      return { success: true, sentTo };
    } catch (error) {
  ...
  sendToConnection(
    ...
    try {
      ...
      return {
        success: false,
        error: 'Connection not subscribed to event type',
      };
    } catch (error) {
   ...
   */
  private sendToConnectionInternal(
    connection: RealtimeConnection,
    event: RealtimeEvent
  ): void {
    try {
  ...
  getEventsForPolling(
    connectionId: string,
    lastEventId?: string
  ): { success: boolean; events?: RealtimeEvent[]; error?: string } {
    try {
  ...
  setUseWebSocket(useWebSocket: boolean): void {
    ...
    console.log(
      `RealtimeIntegration: ${useWebSocket ? 'Using WebSocket' : 'Using polling'}`
    );
  }
