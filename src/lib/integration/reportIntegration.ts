...
export class ReportIntegration {
   ...
   */
  async generateAssetReport(): Promise<{
    success: boolean;
    report?: any;
    error?: string;
  }> {
    try {
      ...
      if (this.useDatabase) {
        ...
        investments = investmentsResult.success
          ...
          : userAuthService.getAllInvestments();
      } else {
   ...
   */
  async generateInvestmentReport(): Promise<{
    success: boolean;
    report?: any;
    error?: string;
  }> {
  ...
  async exportReportToPDF(
    ...
    try {
      ...
      if (!reportResult.success || !reportResult.report) {
        return {
          success: false,
          error: 'Failed to generate report for PDF export',
        };
      }
  ...
  async exportReportToCSV(
    ...
    try {
      ...
      if (!reportResult.success || !reportResult.report) {
        return {
          success: false,
          error: 'Failed to generate report for CSV export',
        };
      }
  ...
  private applyFilters(data: any, filters: any) {
    ...
    if (filters.startDate || filters.endDate) {
      ...
      if (startDate || endDate) {
        filteredData.investments = filteredData.investments.filter(
          (inv: any) => {
    ...
    if (filters.userRole) {
      filteredData.users = filteredData.users.filter(
        (user: any) => user.role === filters.userRole
      );
    }

    // Apply asset type filters
    if (filters.assetType) {
      filteredData.assets = filteredData.assets.filter(
        (asset: any) => asset.type === filters.assetType
      );
    }
  ...
  setUseDatabase(useDatabase: boolean): void {
    ...
    console.log(
      `ReportIntegration: ${useDatabase ? 'Using database' : 'Using mock data'}`
    );
  }
