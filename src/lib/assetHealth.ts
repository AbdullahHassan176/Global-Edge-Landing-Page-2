...
class MockTokensClient {
  async getTimeline(
    ...
    try {
      ...
      const hoursAgo = isRecent
        ? Math.floor(Math.random() * 24)
        : Math.floor(Math.random() * 72) + 24;
...
export async function getAssetHealth(assetKey: string): Promise<AssetHealth> {
  try {
    ...
    const lastEvent = timeline.find(
      event => event.event.eventType === 'OracleSignature'
    );
    ...
    if (lastEvent) {
      ...
      const now = Date.now();
      const hoursSinceLastSignature =
        (now - lastSignatureTime) / (1000 * 60 * 60);
    ...
    const exceptions = await tokensClient.getExceptions({
      assetKey,
      status: 'open',
    });
