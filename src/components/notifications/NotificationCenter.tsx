...
export default function NotificationCenter({
  ...
  const [highPriorityExceptions, setHighPriorityExceptions] = useState<
  ...
  const [regularNotifications] = useState([
    ...
    },
    {
      ...
      message:
        'Your investment in Property Token #PT-2024-003 has been confirmed',
    ...
    try {
      ...
      const highPriorityExceptions = response.exceptions
        ...
        .map(exc => ({
          ...
          friendlyDescription:
            EXCEPTION_TYPE_MAPPING[exc.type]?.description || exc.message,
          severityColor:
            SEVERITY_COLORS[exc.severity] || SEVERITY_COLORS.MEDIUM,
            ...
            {loading ? (
              <div className='p-4 text-center'>
                ...
                <p className='text-sm text-gray-500 mt-2'>
                  Loading high priority notifications...
                </p>
              </div>
            ) : error ? (
              <div className='p-4 text-center'>
                <Icon
                  ...
                  className='h-5 w-5 mx-auto text-red-400'
                />
              ...
              <>
                ...
                {highPriorityExceptions.map(exception => (
                  <div
                    key={exception.id}
                    className='p-4 border-b border-red-100 bg-red-50/50 hover:bg-red-50'
                  >
                    <div className='flex items-start justify-between'>
                      <div className='flex-1'>
                        <div className='flex items-center mb-2'>
                          ...
                          />
                          <span
                            className={`px-2 py-1 text-xs font-medium rounded-full ${exception.severityColor}`}
                          >
                            {exception.severity}
                          </span>
                        </div>
