...
function AssetsPageContent() {
  ...
  useEffect(() => {
    ...
    if (
      category &&
      ['containers', 'property', 'tradetokens', 'vault', 'all'].includes(
        category
      )
    ) {
  ...
  const getCategoryData = () => {
    ...
    const filteredAssets = assets.filter(asset => {
      ...
      if (activeCategory !== 'all') {
        if (activeCategory === 'tradetokens' && asset.type !== 'inventory')
          return false;
        if (activeCategory !== 'tradetokens' && asset.type !== activeCategory)
          return false;
      }
      ...
      if (
        filterRisk !== 'all' &&
        asset.risk.toLowerCase() !== filterRisk.toLowerCase()
      ) {
        return false;
      }
      ...
      if (filterIssuer !== 'all') {
        ...
        const assetIssuer =
          mockIssuers[Math.floor(Math.random() * mockIssuers.length)];
    ...
    filteredAssets.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'lowest-risk':
          ...
          return (
            (riskOrder[a.risk as keyof typeof riskOrder] || 2) -
            (riskOrder[b.risk as keyof typeof riskOrder] || 2)
          );
        case 'newest-first':
          return (
              ...
              {[
                {
                  ...
                  icon: 'ship',
                  color: 'blue',
                  count: assets.filter(a => a.type === 'container').length,
                },
                {
                  key: 'property',
                  label: 'Property',
                  icon: 'building',
                  color: 'green',
                  count: assets.filter(a => a.type === 'property').length,
                },
                {
                  key: 'tradetokens',
                  label: 'TradeTokens',
                  icon: 'boxes',
                  color: 'purple',
                  count: assets.filter(a => a.type === 'inventory').length,
                },
                {
                  key: 'vault',
                  label: 'Vault',
                  icon: 'vault',
                  color: 'orange',
                  count: assets.filter(a => a.type === 'vault').length,
                },
                {
                  key: 'all',
                  label: 'All Assets',
          ...
          <div className='flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0'>
            <div className='flex items-center space-x-4'>
              ...
              >
                <Icon
                  name='filter'
                  className={`w-5 h-5 ${showFilters ? 'text-white' : 'text-gray-600'}`}
                />
                <span
                  className={`font-medium ${showFilters ? 'text-white' : 'text-gray-700'}`}
                >
                  Filters
                </span>
                {(filterRisk !== 'all' ||
              ...
              <div className='flex items-center space-x-2 bg-gray-100 rounded-xl p-1'>
                <button
                  ...
                  className={`p-2 rounded-lg transition-all duration-200 ${
                    viewMode === 'grid'
                      ? 'bg-white text-global-teal shadow-sm'
                      : 'text-gray-600 hover:text-global-teal'
                  }`}
                >
            ...
            <div className='grid md:grid-cols-2 lg:grid-cols-5 gap-6'>
              ...
              <div>
                ...
                <input
                  ...
                  value={filterMinApr}
                  onChange={e =>
                    setFilterMinApr(parseFloat(e.target.value) || 0)
                  }
                  min='0'
                  max='50'
                  step='0.1'
          ...
          <div className='flex items-center justify-between mb-8'>
            ...
            <div className='flex items-center space-x-4'>
              ...
              >
                <span>
                  View All{' '}
                  {categoryData.title === 'All Assets'
                    ? 'Assets'
                    : categoryData.title.replace(' Assets', 's')}
                </span>
                ...
                >
                  <div className='relative'>
                    <Image
                      ...
                      src={
                        asset.image ||
                        'https://storage.googleapis.com/uxpilot-auth.appspot.com/737a82cfea-8505609552f3f2bb8533.png'
                      }
                    ...
                    <div className='absolute top-4 right-4'>
                      <span className='bg-white bg-opacity-95 backdrop-blur-sm px-3 py-2 rounded-full text-xs font-semibold text-gray-700 border border-gray-200'>
                        <Icon
                          name='location-dot'
                          className='text-blue-600 mr-1 w-3 h-3'
                        />
                        {asset.route}
                      </span>
                    </div>
                  ...
                  <div className='p-6'>
                    ...
                    <div className='flex items-start justify-between mb-4'>
                      <div className='flex-1'>
                        ...
                        <p className='text-sm text-gray-600 mb-3'>
                          {asset.cargo} • {asset.type}
                        </p>
                      </div>
                      <div className='text-right'>
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-bold border ${getAprBadgeColor(asset.apr)}`}
                        >
                          {asset.apr}% APR
                        </span>
                      </div>
                    ...
                    <div className='grid grid-cols-2 gap-4 mb-6'>
                      ...
                      <div className='bg-gray-50 rounded-lg p-3'>
                        ...
                        <span
                          className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${getRiskBadgeColor(asset.risk)}`}
                        >
                          {asset.risk}
                        </span>
                      </div>
                    ...
                    <div className='mb-6'>
                      ...
                      <div className='w-full bg-gray-200 rounded-full h-3'>
                        <div
                          className='bg-gradient-to-r from-global-teal to-edge-purple h-3 rounded-full transition-all duration-500'
                          style={{ width: '87%' }}
                        ></div>
                      </div>
                    </div>
                      ...
                      <div className='flex items-start justify-between mb-4'>
                        <div className='flex-1'>
                          ...
                          <p className='text-gray-600 mb-3'>
                            {asset.cargo} • {asset.type} • {asset.route}
                          </p>
                          <div className='flex items-center space-x-4'>
                            <div className='flex items-center space-x-1'>
                              <Icon
                        ...
                        <div className='text-right'>
                          <span
                            className={`inline-flex items-center px-4 py-2 rounded-full text-lg font-bold border ${getAprBadgeColor(asset.apr)}`}
                          >
                            {asset.apr}% APR
                          </span>
                        </div>
                      ...
                      <div className='grid grid-cols-5 gap-6 mb-6'>
                        ...
                        <div className='bg-gray-50 rounded-lg p-4'>
                          ...
                          <div className='w-full bg-gray-200 rounded-full h-2 mt-1'>
                            <div
                              className='bg-gradient-to-r from-global-teal to-edge-purple h-2 rounded-full transition-all duration-500'
                              style={{ width: '87%' }}
                            ></div>
                          </div>
                        </div>
      ...
      <section className='py-12 bg-gray-50'>
        <div className='max-w-7xl mx-auto px-6 lg:px-8'>
          <h2 className='text-3xl font-poppins font-bold text-charcoal mb-8 text-center'>
            Explore Other Asset Categories
          </h2>
          ...
          <div className='grid md:grid-cols-3 gap-8'>
            ...
            <div className='bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow group cursor-pointer'>
              ...
              <p className='text-gray-600 mb-6'>
                ...
                with rental income streams and long-term appreciation potential.
                Learn about our{' '}
                <Link
                  href='/how-it-works'
                  className='text-global-teal hover:text-edge-purple transition-colors'
            ...
            <div className='bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow group cursor-pointer'>
              ...
              <p className='text-gray-600 mb-6'>
                ...
                with short-term returns from global supply chain movements and
                commodity trading. Discover our{' '}
                <Link
                  href='/how-it-works'
                  className='text-global-teal hover:text-edge-purple transition-colors'
                >
            ...
            <div className='bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow group cursor-pointer'>
              ...
              <p className='text-gray-600 mb-6'>
                ...
                , gems, and high-value assets with insurance-backed protection
                and stable returns. Explore our{' '}
                <Link
                  href='/how-it-works'
                  className='text-global-teal hover:text-edge-purple transition-colors'
                >
