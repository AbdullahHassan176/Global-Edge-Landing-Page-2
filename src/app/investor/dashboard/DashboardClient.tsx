...
function InvestmentItem({ investment }: { investment: Investment }) {
  ...
  useEffect(() => {
    const loadAssetDetails = async () => {
      try {
        const assetResponse = await databaseService.getAssetById(
          investment.assetId
        );
        if (assetResponse.success && assetResponse.data) {
          ...
          const localAsset = await assetService.getAssetById(
            investment.assetId
          );
          if (localAsset) {
        ...
        console.log(
          'Could not load asset details for investment:',
          investment.id
        );
        setAssetName('Unknown Asset');
      }
    ...
    try {
      ...
      try {
        const notificationsResponse =
          await databaseService.getNotificationsByUserId(currentUser.id);
        if (notificationsResponse.success && notificationsResponse.data) {
          userNotifications = notificationsResponse.data;
        }
      } catch (dbError) {
        console.log('Database not available, using local notifications');
        userNotifications = await userAuthService.getNotifications(
          currentUser.id
        );
      }
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-gray-600 mt-1'>
                Access and manage your portfolio of tokenized assets in real
                time.
              </p>
            </div>
            <div className='flex items-center space-x-4'>
            ...
            <div className='flex items-center'>
              ...
              <div>
                ...
                <p className='text-yellow-700 text-sm'>
                  Complete your KYC verification to start investing in tokenized
                  assets.
                </p>
        ...
        <div className='mb-8'>
          <HoldingsTable
            holdings={investments.map(inv => ({
              ...
              currentValue: inv.amount * 1.05,
              returns: inv.amount * 0.05,
            }))}
          />
        </div>
          ...
          <div className='bg-white rounded-xl shadow-sm border border-gray-200'>
            <div className='p-6 border-b border-gray-200'>
              <Tooltip content='Your latest investment transactions showing status and amounts'>
                <h2 className='text-xl font-semibold text-gray-900 cursor-help'>
                  Recent Investments
                </h2>
              </Tooltip>
            </div>
            <div className='p-6'>
              {investments.length === 0 ? (
                <div className='text-center py-8'>
                  <Icon
                    name='inbox'
                    className='text-gray-400 text-3xl mx-auto mb-3'
                  />
                    ...
                    <div
                      ...
                      className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                        notification.read
                          ...
                          : 'border-global-teal/20 bg-global-teal/5'
                      }`}
                      onClick={() =>
