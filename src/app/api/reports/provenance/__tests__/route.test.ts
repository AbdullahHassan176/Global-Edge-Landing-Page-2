  ...
  describe('GET', () => {
    it('should return 400 when assetKey is missing', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/reports/provenance'
      );
    ...
    it('should generate PDF with valid assetKey', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/reports/provenance?assetKey=test-asset-123'
      );
      const response = await GET(request);
      ...
      expect(response.headers.get('Content-Disposition')).toContain(
        'provenance-report-test-asset-123.pdf'
      );
    });
    ...
    it('should include proper headers for PDF download', async () => {
      ...
      expect(response.headers.get('Content-Disposition')).toContain(
        'filename="provenance-report-test-asset-123.pdf"'
      );
