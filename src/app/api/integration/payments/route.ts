    ...
    switch (type) {
      ...
      default:
        return NextResponse.json(
          {
            success: false,
            error: 'Invalid type parameter. Use: methods, history, or status',
          },
          { status: 400 }
        );
    }
  ...
  try {
    ...
    switch (action) {
      case 'create_payment_intent':
        ...
        result = await paymentIntegration.createPaymentIntent(
          amount,
          currency,
          paymentMethod,
          description,
      ...
      case 'process_stripe_payment':
        ...
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: 'Missing required fields for Stripe payment',
            },
            { status: 400 }
          );
        }
        result = await paymentIntegration.processStripePayment(
          stripeAmount,
          stripeCurrency,
          paymentMethodId,
          stripeDescription,
      ...
      case 'process_paypal_payment':
        ...
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: 'Missing required fields for PayPal payment',
            },
            { status: 400 }
          );
        }
        result = await paymentIntegration.processPayPalPayment(
          paypalAmount,
          paypalCurrency,
          paypalDescription,
          paypalMetadata
      ...
      case 'add_payment_method':
        ...
        if (!userId || !methodData) {
          return NextResponse.json(
            {
              success: false,
              error: 'Missing required fields for adding payment method',
            },
            { status: 400 }
          );
        }
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Invalid action. Use: create_payment_intent, process_stripe_payment, process_paypal_payment, or add_payment_method',
          },
          { status: 400 }
        );
    }
