...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (reportType) {
      ...
      case 'custom':
        result = await reportIntegration.generateCustomReport(
          'custom',
          filters
        );
        break;
    ...
    if (format === 'pdf') {
      const pdfResult = await reportIntegration.exportReportToPDF(
        reportType,
        filters
      );
    ...
    if (format === 'csv') {
      const csvResult = await reportIntegration.exportReportToCSV(
        reportType,
        filters
      );
