  ...
  try {
    ...
    switch (type) {
      ...
      case 'metrics':
        ...
        result = await performanceIntegration.getMetricsByCategory(
          category,
          timeRange
        );
        break;
  ...
  try {
    ...
    switch (action) {
      case 'record_metric':
        ...
        if (!name || value === undefined || !unit || !metricCategory) {
          return NextResponse.json(
            {
              success: false,
              error: 'name, value, unit, and category are required',
            },
            { status: 400 }
          );
        }
        result = await performanceIntegration.recordMetric(
          name,
          value,
          unit,
          metricCategory,
          ...
          return NextResponse.json(
            {
              success: false,
              error:
                'endpoint, method, responseTime, and statusCode are required',
            },
            { status: 400 }
          );
        }
        result = await performanceIntegration.recordApiResponseTime(
          endpoint,
          method,
          responseTime,
          statusCode
      ...
      case 'record_database_query':
        ...
        if (
          ...
          return NextResponse.json(
            {
              success: false,
              error: 'query, executionTime, and rowsAffected are required',
            },
            { status: 400 }
          );
        }
        result = await performanceIntegration.recordDatabaseQueryTime(
          query,
          executionTime,
          rowsAffected
        );
      ...
      case 'record_user_activity':
        ...
        result = await performanceIntegration.recordUserActivity(
          userId,
          userAction,
          duration
        );
      ...
      case 'record_business_metric':
        ...
        result = await performanceIntegration.recordBusinessMetric(
          businessName,
          businessValue,
          businessUnit,
          businessTags
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              'Invalid action. Use: record_metric, record_api_response, record_database_query, record_user_activity, or record_business_metric',
          },
          { status: 400 }
        );
    }
