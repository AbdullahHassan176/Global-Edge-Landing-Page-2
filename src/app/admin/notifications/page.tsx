...
function NotificationsDashboard() {
  ...
  const [filter, setFilter] = useState<
    'all' | 'new' | 'reviewed' | 'processed'
  >('all');
  const [activeTab, setActiveTab] = useState<
    'admin' | 'emails' | 'webhooks' | 'stats'
  >('admin');
  ...
  const formatEmailTemplate = (template: string, data: any) => {
    ...
    if (data.verificationLink)
      formattedTemplate = formattedTemplate.replace(
        /\{\{verificationLink\}\}/g,
        data.verificationLink || 'https://example.com/verify'
      );
    if (data.loginLink)
      formattedTemplate = formattedTemplate.replace(
        /\{\{loginLink\}\}/g,
            ...
            <div className='flex flex-wrap justify-center gap-4 mb-8'>
              {[
                ...
                },
                {
                  key: 'webhooks',
                  label: 'Webhooks',
                  count: webhookNotifications.length,
                },
                ...
                <div className='flex flex-wrap justify-center gap-4 mb-8'>
                  {[
                    ...
                    },
                    {
                      key: 'new',
                      label: 'New',
                      count: notifications.filter(n => n.status === 'new')
                        .length,
                    },
                    {
                      key: 'reviewed',
                      label: 'Reviewed',
                      count: notifications.filter(n => n.status === 'reviewed')
                        .length,
                    },
                    {
                      key: 'processed',
                      label: 'Processed',
                      count: notifications.filter(n => n.status === 'processed')
                        .length,
                    },
                  ].map(({ key, label, count }) => (
                    <button
                        ...
                        <div className='flex items-start space-x-4'>
                          ...
                          <div className='flex-1'>
                            <div className='flex items-center space-x-3 mb-2'>
                              ...
                              <span
                                className={`px-3 py-1 rounded-full text-sm font-medium border ${getPriorityColor(notification.priority)}`}
                              >
                      ...
                      <div className='flex items-start space-x-4'>
                        ...
                        <div className='flex-1'>
                          <div className='flex items-center space-x-3 mb-2'>
                            ...
                            <span
                              className={`px-3 py-1 rounded-full text-sm font-medium ${getEmailStatusColor(email.status)}`}
                            >
              ...
              <div className='space-y-6'>
                      ...
                      <div className='flex items-start space-x-4'>
                        ...
                        <div className='flex-1'>
                          <div className='flex items-center space-x-3 mb-2'>
                            ...
                            <span
                              className={`px-3 py-1 rounded-full text-sm font-medium ${getWebhookStatusColor(webhook.status)}`}
                            >
                ...
                {webhookNotifications.length === 0 && (
                  <div className='text-center py-12'>
                    ...
                    <p className='text-gray-500'>
                      No webhook notifications have been sent yet.
                    </p>
                  </div>
                )}
              </div>
                    ...
                    <div className='bg-white rounded-2xl p-6 shadow-lg'>
                      ...
                      <div className='space-y-4'>
                        <div className='flex justify-between'>
                          ...
                          <span className='font-semibold'>
                            {stats?.webhookStats?.total || 0}
                          </span>
                        </div>
                        <div className='flex justify-between'>
                          <span className='text-gray-600'>Sent:</span>
                          <span className='font-semibold text-green-600'>
                            {stats?.webhookStats?.sent || 0}
                          </span>
                        </div>
                        <div className='flex justify-between'>
                          <span className='text-gray-600'>Failed:</span>
                    ...
                    <div className='bg-white rounded-2xl p-6 shadow-lg'>
                      ...
                      <div className='space-y-4'>
                        <div className='flex justify-between'>
                          ...
                          <span className='font-semibold'>
                            {stats?.adminStats?.total || 0}
                          </span>
                        </div>
                        <div className='flex justify-between'>
                          <span className='text-gray-600'>New:</span>
                          <span className='font-semibold text-red-600'>
                            {stats?.adminStats?.new || 0}
                          </span>
                        </div>
                        <div className='flex justify-between'>
                          <span className='text-gray-600'>Reviewed:</span>
                          <span className='font-semibold text-yellow-600'>
                            {stats?.adminStats?.reviewed || 0}
                          </span>
                        </div>
                        <div className='flex justify-between'>
                          <span className='text-gray-600'>Processed:</span>
                          <span className='font-semibold text-green-600'>
                            {stats?.adminStats?.processed || 0}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className='bg-gray-50 rounded-lg p-4'>
                    <div className='grid md:grid-cols-2 gap-4 text-sm'>
                      ...
                      <div>
                        ...
                        <span
                          className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getEmailStatusColor(selectedEmail.status)}`}
                        >
                ...
                <div className='flex space-x-3'>
                  ...
                  <button
                    onClick={() => {
                      const newWindow = window.open('', '_blank');
                      if (newWindow) {
                        const formattedContent = formatEmailTemplate(
                          selectedEmail.template,
                          selectedEmail.data
                        );
                        newWindow.document.write(`
