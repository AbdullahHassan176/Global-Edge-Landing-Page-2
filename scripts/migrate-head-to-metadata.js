...
function extractMetadata(content) {
  ...
  // Extract keywords
  const keywordsMatch = headContent.match(/<meta name="keywords" content="(.*?)" \/>/);
  if (keywordsMatch) metadata.keywords = keywordsMatch[1];

  // Extract canonical
  const canonicalMatch = headContent.match(/<link rel="canonical" href="(.*?)" \/>/);
  if (canonicalMatch) metadata.canonical = canonicalMatch[1];

  // Extract Open Graph
  const ogTitleMatch = headContent.match(/<meta property="og:title" content="(.*?)" \/>/);
  const ogDescMatch = headContent.match(/<meta property="og:description" content="(.*?)" \/>/);
  const ogUrlMatch = headContent.match(/<meta property="og:url" content="(.*?)" \/>/);
  const ogImageMatch = headContent.match(/<meta property="og:image" content="(.*?)" \/>/);
  ...
  // Extract Twitter
  const twitterTitleMatch = headContent.match(/<meta name="twitter:title" content="(.*?)" \/>/);
  const twitterDescMatch = headContent.match(/<meta name="twitter:description" content="(.*?)" \/>/);
  const twitterImageMatch = headContent.match(/<meta name="twitter:image" content="(.*?)" \/>/);
  ...
  // Extract JSON-LD
  const jsonLdMatch = headContent.match(/<script[\s\S]*?type="application\/ld\+json"[\s\S]*?dangerouslySetInnerHTML={{\s*__html: JSON\.stringify\(([\s\S]*?)\)\s*}}[\s\S]*?\/>/);
  if (jsonLdMatch) {
    try {
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (metadata.openGraph) {
    ...
    openGraph: {
      ...baseMeta.openGraph,`;
    if (metadata.openGraph.title) layoutContent += `
      title: '${metadata.openGraph.title}',`;
    if (metadata.openGraph.description) layoutContent += `
      description: '${metadata.openGraph.description}',`;
    if (metadata.openGraph.url) layoutContent += `
      url: '${metadata.openGraph.url}',`;
    if (metadata.openGraph.type) layoutContent += `
  ...
  if (metadata.twitter) {
    ...
    twitter: {
      ...baseMeta.twitter,`;
    if (metadata.twitter.title) layoutContent += `
      title: '${metadata.twitter.title}',`;
    if (metadata.twitter.description) layoutContent += `
      description: '${metadata.twitter.description}',`;
    if (metadata.twitter.card) layoutContent += `
