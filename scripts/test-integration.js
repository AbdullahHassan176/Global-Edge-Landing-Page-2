    ...
    try {
      ...
      if (response.status === 200) {
        results.server = {
          port,
          status: 'running',
          url: `${BASE_URL.replace('3000', port)}`,
        };
...
async function testDatabaseConnection(port) {
  ...
  try {
    const response = await makeRequest(
      `${BASE_URL.replace('3000', port)}/api/database/test`
    );
    results.database = {
    ...
    if (response.status === 200) {
      ...
      console.log(
        `   Database: ${response.data.data?.stats?.databaseId || 'Unknown'}`
      );
      console.log(
        `   Containers: ${response.data.data?.stats?.containerCount || 0}`
      );
    } else {
...
async function testAPIEndpoints(port) {
  ...
  for (const endpoint of endpoints) {
    try {
      const response = await makeRequest(
        `${BASE_URL.replace('3000', port)}${endpoint.url}`
      );
...
function generateSuggestions() {
  ...
  if (results.database?.status === 'connected') {
    results.suggestions.push({
      ...
      priority: 'High',
      suggestion:
        'Database is connected! You can now integrate real data with your existing services.',
      implementation:
        'Replace mock data in userAuthService, assetService, and notificationService with database calls',
    });
    ...
    results.suggestions.push({
      ...
      implementation:
        'Update src/lib/userAuthService.ts to use databaseService.createUser() and databaseService.getUserByEmail()',
    });
    ...
    results.suggestions.push({
      ...
      implementation:
        'Update src/lib/assetService.ts to use databaseService.createAsset() and databaseService.getAssets()',
    });
  } else {
    results.suggestions.push({
      category: 'Database',
      ...
      implementation:
        'Check environment variables, Cosmos DB credentials, and network connectivity',
    });
  }
  ...
  if (results.api['Users API']?.status === 'working') {
    results.suggestions.push({
      ...
      implementation:
        'Update src/app/admin/users/page.tsx to use /api/users endpoint instead of mock data',
    });
  }
  ...
  results.suggestions.push({
    ...
    implementation:
      'Create JWT tokens, session management, and integrate with existing login flows',
  });
  ...
  results.suggestions.push({
    ...
    implementation:
      'Implement WebSocket connections or Server-Sent Events for live updates',
  });
  ...
  results.suggestions.push({
    ...
    implementation:
      'Add Azure Blob Storage integration for document and image storage',
  });
  ...
  results.suggestions.push({
    ...
    implementation:
      'Update notificationService to store and track email delivery status in database',
  });
  ...
  results.suggestions.push({
    ...
    implementation:
      'Create analytics dashboard with investment tracking, user behavior, and performance metrics',
  });
  ...
  results.suggestions.push({
    ...
    implementation:
      'Add Stripe or PayPal integration for investment payments and fee collection',
  });
  ...
  results.suggestions.push({
    ...
    implementation:
      'Develop React Native or Flutter app with same API endpoints',
  });
}
...
function printResults() {
  ...
  if (results.database) {
    console.log(
      `   Status: ${results.database.status === 'connected' ? '✅ Connected' : '❌ Error'}`
    );
  ...
  console.log(
    '2. Implement high-priority integrations (User management, Asset management)'
  );
  console.log(
    '3. Add medium-priority features (Admin dashboard, Real-time updates)'
  );
